...
def prepare_data_seq(batch_size=100):
    from utils.domain_split import (
        domain_policy_active,
        filter_dialogue_pairs,
        print_domain_policy_summary,
        resolve_data_paths,
    )

    file_train, file_dev, file_test = resolve_data_paths(
        args.get("dataset", "woz"),
        args.get("train_file", ""),
        args.get("dev_file", ""),
        args.get("test_file", ""),
    )
    ...
    seed = int(args.get("random_seed", 1234))
    td = (args.get("target_domain") or "").strip()
    tr = float(args.get("target_ratio", 1.0) or 1.0)
    gtr = float(args.get("train_ratio", 1.0) or 1.0)
    xd = (args.get("exclude_domains") or "").strip()
    dev_x = bool(args.get("dev_exclude_target"))

    if domain_policy_active(args):
        pair_train, st = filter_dialogue_pairs(
            pair_train, "woz",
            target_domain=td, target_ratio=tr, train_ratio=gtr, exclude_domains=xd,
            dev_exclude_target=False, split="train", seed=seed,
        )
        pair_dev, sd = filter_dialogue_pairs(
            pair_dev, "woz",
            target_domain=td, target_ratio=tr, train_ratio=gtr, exclude_domains=xd,
            dev_exclude_target=dev_x, split="dev", seed=seed,
        )
        pair_test, tt = filter_dialogue_pairs(
            pair_test, "woz",
            target_domain="", target_ratio=1.0, exclude_domains="",
            dev_exclude_target=False, split="test", seed=seed,
        )
        print_domain_policy_summary(st, sd, tt)
    else:
        print("Reading data: %s | %s | %s" % (file_train, file_dev, file_test))
